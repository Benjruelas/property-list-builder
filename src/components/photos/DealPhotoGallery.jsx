...
export function DealPhotoGallery({
  ...
  const photoPreviewItems = useMemo(
    () => photos.map((photo, index) => ({
      ...
      photo,
      loadBlob: () => fetchDealPhotoBlob(getToken, photo.key),
    })),
    [photos, getToken],
  )
  ...
  const loadThumb = useCallback(async (photo) => {
    ...
    try {
      ...
      if (!token) return
      const res = await fetch(dealPhotoUrl(key), { headers: { Authorization: `Bearer ${token}` } })
      if (!res.ok) return
  ...
  const handlePhotosUploaded = (updatedDeal) => {
  ...
  const handleDelete = async (photo) => {
    ...
    try {
      const { deal: updated } = await deleteDealPhoto(getToken, {
        pipelineId,
        dealId: deal.id,
        photoId: photo.id,
      })
  ...
  const handleAnnotatorSave = (updatedDeal) => {
    onDealUpdate?.(updatedDeal)
    setAnnotating(null)
  }
